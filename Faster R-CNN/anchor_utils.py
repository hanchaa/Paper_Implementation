...
def create_anchors_label(anchors: Tensor, ious: Tensor, pos_iou_threshold: float,
    ...
    anchors_label = torch.full((anchors.shape[0],), -1, device=anchors.device, dtype=torch.int8)

    # first condition
    gt_max_iou_anchor = torch.argmax(ious, dim=0)
    anchors_label[gt_max_iou_anchor] = 1

    # second condition
    anchors_max_ious = torch.amax(ious, dim=1)
    pos_iou_anchors = torch.where(anchors_max_ious >= pos_iou_threshold)[0]
    neg_iou_anchors = torch.where(anchors_max_ious < neg_iou_threshold)[0]
...
def sample_anchors(anchors_label: Tensor, num_sample: int, pos_ratio: float) -> Tensor:
    positive_anchor_indexes = torch.where(anchors_label == 1)[0]
    negative_anchor_indexes = torch.where(anchors_label == 0)[0]
    ...
    if total_num_pos > num_sample * pos_ratio:
        sample = torch.multinomial(torch.ones_like(positive_anchor_indexes).float(),
                                   num_samples=total_num_pos - num_pos_sample)
        disable_index = positive_anchor_indexes[sample]
        anchors_label[disable_index] = -1

    sample = torch.multinomial(torch.ones_like(negative_anchor_indexes).float(),
                               num_samples=len(negative_anchor_indexes) - num_neg_sample)
...
def create_target_anchors(num_anchors: int, num_sample: int, valid_anchors: Tensor,
    ...
    target_anchors_label = torch.full((num_anchors,), -1, dtype=torch.int8, device=valid_anchors.device)
    ...
    target_anchors_reg_parameter = torch.zeros((num_anchors, 4), dtype=torch.float32, device=valid_anchors.device)
